...
<<<<<<< HEAD
        ...
        for i in range(1,len(ret)+1):
=======
        #fps 확인 단계, 기본적으로 10000fps 촬영하나 용량에따라 저장시 skip필요. 이럴경우 실질적인 fps달라지기때문에 해당절차진행
        #대신, 파일명에 10skip과 같은형태로 기록이 되어있어야 인식해서 fps 반영가능함.     
        time = []
        fps_skip=[]
        fps_skip2=[]
        if 'skip' in savenumber:
            fps_skip.append(1)
            if len(fps_skip) != 0:
                for i in range(1,50): ##50skip 까지만 확인 가능
                    if '{}skip'.format(i) in savenumber:
                        fps_skip2.append(i)
        else:
            fps_skip2= [1]

        for i in range(1,len(ret)+1):
            fps = 10000 / fps_skip2[0]
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
            t = i /fps
            time.append(t)
        data = {'time':time, 'Start':ret_s, 'Break':ret, 'width(fixed)':ret1, 'width(min)':ret2, 'widht(min_mov)' :ret3}
...
=======
    def Dos_CaBER_VE_analysis(self,tifname,surfacet):
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
        """
...
=======
        ...
        directory_1 = os.getcwd()+'\\'+tifname[:-5]
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
        image_check = self.get_nth_image(-1)
        image_c = cv2.Canny(image_check,50,220)
        yss, xss = np.where(image_c == 255)    
...
=======
        Measurement_data = pd.read_excel('{}\{}'.format(directory_1,tifname),header=0,index_col=0)
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
<<<<<<< HEAD
        Measurement_data = pd.read_excel('{}/{}'.format(directory_2,fname),header=0,index_col=0,engine='openpyxl')
=======
        Measurement_data = pd.read_excel('{}\{}'.format(directory_2,fname),header=0,index_col=0)
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
        time = Measurement_data['time'].values.tolist()
        ret1 = Measurement_data['width(fixed)'].values.tolist()
        ret2 = Measurement_data['width(min)'].values.tolist()
...
=======
                slope = -(ret1_array_unitcontrol[i]-ret1_array_unitcontrol[i-term])/(ret1_array_unitcontrol[i]) / (xdata[i]-xdata[i-term])
                ret1_strainrate.append(slope)
            else:
                slope = -(ret1_array_unitcontrol[term]-ret1_array_unitcontrol[0])/(ret1_array_unitcontrol[term])/(xdata[term]-xdata[0])
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
                ret1_strainrate.append(slope)
        ret2_strainrate = []
        for i in range(len(xdata)):
...
=======
                slope = -(ret2_array_unitcontrol[i]-ret2_array_unitcontrol[i-term])/(ret2_array_unitcontrol[i]) / (xdata[i]-xdata[i-term])
                ret2_strainrate.append(slope)
            else:
                slope = -(ret2_array_unitcontrol[term]-ret2_array_unitcontrol[0])/(ret2_array_unitcontrol[term])/(xdata[term]-xdata[0])
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
                ret2_strainrate.append(slope)
        ret3_strainrate = []
        for i in range(len(xdata)):
...
=======
                slope = -(ret3_array_unitcontrol[i]-ret3_array_unitcontrol[i-term])/(ret3_array_unitcontrol[i]) / (xdata[i]-xdata[i-term])
                ret3_strainrate.append(slope)
            else:
                slope = -(ret3_array_unitcontrol[term]-ret3_array_unitcontrol[0])/(ret3_array_unitcontrol[term])/(xdata[term]-xdata[0])
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
                ret3_strainrate.append(slope)
...
=======
        data = {'time':time_slice,'width(fixed)':ret1_array_unitcontro, 'width(min)':ret2_array_unitcontro, 'widht(min_mov)' :ret3_array_unitcontro, 'ne(fixed)':ret1_extensional_v,'ne(min)':ret2_extensional_v,'ne(min_sort)':ret3_extensional_v}
        data = pd.DataFrame(data)
        with pd.ExcelWriter('{}\{}'.format(directory_2,fname),engine='openpyxl',mode='a') as writer:
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax4.set_title("{}fixed_roi_extensional viscoisty".format(fname[:-5]))
        ax4.set_ylabel("ηE")
        ax4.set_xlabel("R")
        fig.savefig('{}\{}'.format(directory_2,"{}fixed_roi_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax5.set_title("{}min_extensional viscoisty".format(fname[:-5]))
        ax5.set_ylabel("ηE")
        ax5.set_xlabel("R")
        fig.savefig('{}\{}'.format(directory_2,"{}min_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax6.set_title("{}min_sort_extensional viscoisty".format(fname[:-5]))
        ax6.set_ylabel("ηE")
        ax6.set_xlabel("R")
        fig.savefig('{}\{}'.format(directory_2,"{}min_sort_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax7.set_title("{}fixed_roi_extensional viscoisty_sr".format(fname[:-5]))
        ax7.set_ylabel("ηE")
        ax7.set_xlabel("Strain Rate")
        fig.savefig('{}\{}'.format(directory_2,"{}fixed_roi_extensional viscoisty_sr".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax8.set_title("{}min_extensional viscoisty_sr".format(fname[:-5]))
        ax8.set_ylabel("ηE")
        ax8.set_xlabel("Strain Rate")
        fig.savefig('{}\{}'.format(directory_2,"{}min_extensional viscoisty_sr".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax9.set_title("{}min_sort_extensional viscoisty_sr".format(fname[:-5]))
        ax9.set_ylabel("ηE")
        ax9.set_xlabel("Strain Rate")
        fig.savefig('{}\{}'.format(directory_2,"{}min_sort_extensional viscoisty_sr".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax10.set_title("{}fixed_roi_extensional viscoisty".format(fname[:-5]))
        ax10.set_ylabel("ηE")
        ax10.set_xlabel("hencky strain")
        fig.savefig('{}\{}'.format(directory_2,"{}_fixed_roi_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax11.set_title("{}min_extensional viscoisty".format(fname[:-5]))
        ax11.set_ylabel("ηE")
        ax11.set_xlabel("hencky strain")
        fig.savefig('{}\{}'.format(directory_2,"{}_min_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        ax12.set_title("{}min_sort_extensional viscoisty".format(fname[:-5]))
        ax12.set_ylabel("ηE")
        ax12.set_xlabel("hencky strain")
        fig.savefig('{}\{}'.format(directory_2,"{}_min_sort_extensional viscoisty".format(fname[:-5])))
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
=======
        path = 'C:/Users/MCPL-JJ/Desktop/codes/mcplexpt/samples/caber/dos'
>>>>>>> fb631c5eb8460ebedb5bb280d305a7259e0c1ee0
...
<<<<<<< HEAD
    ...
    def Image_storage(self,date,frame,weight):
        """
        측정영상(tif file)을 Frame 별로 분석하여 측정시간/ Break시간/ Wdith 값을 엑셀로 추출합니다.
        fps에 따라 code 정보 변경해야 합니다. (기본 10000 fps로 설정)
        ...
        for i in range(0,len(date_list)):
            print(savenumber[i])
            sys.stdout.flush()
            self.image_save(date_list[i],savenumber[i],frame,weight)
   


    def image_save(self,fname,savenumber,frame,weight):
        import os
        import pandas as pd
        import matplotlib.pyplot as plt
        from mcplexpt.caber.dos import DoSCaBERExperiment
        ...
        for i in range(start_f,break_t):
            ...
            elif i > break_t-frame:
             
                            
                if '2skip' in fname:                                      
                    image_c = image_c/8000 *weight*2 ## 8000의 기준은 사실 명확하지 않음. 기본적으로 DoS-CaBER로 측정하는 유체의 Frame이 8000frame안에서 종결될 것이라는 가정하에 선정. 이게 너무 높아지면 빨리 끊어지는 유체의 pixel 값이 너무 작아짐.
                    image_list.append(image_c)           
                
                else:
                    image_c = image_c/8000*weight
                    image_list.append(image_c)
            else:
                image_c = image_c/8000
                image_list.append(image_c)
        image_sum = np.float64(sum(image_list))
        print(np.max(image_sum))         
        sys.stdout.flush()    
        image_sum = np.array(image_sum,dtype=np.float64)        
        #image_sum = image_sum/np.max(image_sum)*255        

        '''if 'PEO,CMC_11_' in fname:
            image_sum=image_sum/800*500
            print('1')
        if 'PEO200_10' in fname:
            image_sum=image_sum/800*700
            print('2')
        if 'PEO,Carbo_11' in fname:
            image_sum=image_sum/800*300
            print('3')
        if 'Carbo5PEO5' in fname:
            image_sum=image_sum/800*300
            print('3')            
        if 'CMC,Carbo_11_'in fname:
            image_sum=image_sum/800*50
            print('4')
        if 'CMC1_CARBOPOL1' in fname:
            image_sum=image_sum/800*50
            print('4')            
        if 'CMC(10)'in fname:
            image_sum=image_sum/800*100
            print('5')
        if '_CMC_01' in fname:
            image_sum=image_sum/800*100   
            print('5')        
        if 'PEO,CMC,Carbo_111' in fname:
            image_sum=(image_sum)/800*400
            print('6')'''
        ...
        cv2.imwrite('/home/minhyukim/pcalist/'+savenumber+'_multi_image_final_{}frame_{}weight.png'.format(frame,weight),image_sum)
